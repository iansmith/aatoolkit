...
<!-- GENERATED from slopstop e09a028 by install-for-project.sh — do not edit.
...
- `.claude/skills/slopstop-run/references/user-output.md` — what the user sees and when. **Quiet by default**:
...
and the final summary. See `.claude/skills/slopstop-run/references/user-output.md`.
...
-> Read `.claude/skills/slopstop-run/references/stages-lean.md` **when `--full` is absent**, before stage 4.
...
**"The same suite" means the same runnable node-id set, not the same count** — compared as sets, in both directions. -> Read `.claude/skills/slopstop-run/references/node-ids.md`
...
1. **The node-id set did not shrink.** Compare sets, not line counts. A dropped node-id stops the ticket on its own. Enumerate both sides from the runner, never from source. -> Read `.claude/skills/slopstop-run/references/node-ids.md`
...
-> Read `.claude/skills/slopstop-run/references/stages-lean.md`
...
-> Read `.claude/skills/slopstop-run/references/stages-review.md`
...
-> Read `.claude/skills/slopstop-run/references/stages-close.md`
